...
export default function SearchlistActions({toggleDisplay, chartinfo}) {
    const songs = useSelector(state => state.songs.songs);
    const dispatch = useDispatch();
    const { theme } = useContext(ThemeContext);
    const [progress, setProgress] = useState(0);
    const [downloadState, setDownloadState] = useState('notyet');
    const [isHiddenDownButton, setIsHiddenDownButton] = useState(false);
    const saveChartlist = (url) => {
        let chartlists = JSON.parse(localStorage.getItem('songs')) || [];
        for (let chartlist of chartlists) {
            if (chartlist.id === chartinfo.id) {
    ...
    const toggleHiddenStatus = () => {
        if (downloadState === 'done') {
            setIsHiddenDownButton(!isHiddenDownButton);
            toggleDisplay();
        }
    }
    useEffect(() => {
    ...
    return (
        <div className='search-list-action'>
            <button className='search-downloadbtn' onClick={startDownload}>
                ...
                {theme === 'dark-theme' && downloadState !== 'done' && !isHiddenDownButton && <svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
